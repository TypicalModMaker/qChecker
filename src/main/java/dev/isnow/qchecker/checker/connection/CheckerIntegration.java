package dev.isnow.qchecker.checker.connection;

import com.google.gson.Gson;
import dev.isnow.qchecker.checker.protocol.json.data.MCResponse;
import dev.isnow.qchecker.checker.protocol.packet.impl.EncryptionPacket;
import dev.isnow.qchecker.checker.protocol.packet.impl.HandshakePacket;
import dev.isnow.qchecker.checker.protocol.packet.impl.LoginStartPacket;
import dev.isnow.qchecker.checker.protocol.packet.impl.MotdHandshakePacket;
...
public class CheckerIntegration {
    ...
    public String connect(String ip, int port, int protocol, int tries, ArrayList<ConnectAtributes> connectAtributes) throws IOException, InterruptedException {
        ...
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        DataOutputStream packetStream = new DataOutputStream(outputStream);
        HandshakePacket handshakePacket = new HandshakePacket(packetStream, connectAtributes, "qChecker", ip, protocol, port);
        handshakePacket.run();
        ...
        LoginStartPacket loginStartPacket = new LoginStartPacket(packetStream, connectAtributes, protocol, "qChecker");
